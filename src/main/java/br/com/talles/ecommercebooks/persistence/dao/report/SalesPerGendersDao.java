...
public class ReportDao extends AbstractDao {
    ...
    public List<Entity> select(boolean enabled, Entity entity) {
        ...
        List<Entity> reports = new ArrayList<>();

        String sql = "select count(c.GENDER) as countSale, c.GENDER "
                + "from sales s "
        ...
        try {
            ...
            while (result.next()) {
                SalesPerGenders salesG = new SalesPerGenders();

                ((CountOfGenders) salesG.getResultQuery()).setSalesAmmount(
                        result.getInt("countSale"));
                ((CountOfGenders) salesG.getResultQuery()).setGender(
                        new Gender(result.getString("customers.gender")));
